...
char *readline(FILE *fp, char *lc, int *lenline){
	char * head; /* the new string */
	...
	head = (ptr = (char *)malloc(sizeof(char)));
	...
	while ((*ptr = fgetc(fp)) != EOF && *ptr != NLN){
		if (pc != BLK || (*ptr != BLK && *ptr != TAB)){ /* skip the current character if there are consecutive white characters */
			char *temp = (char *)realloc((void *) head, ++len * sizeof(char)); /* allocate space for the next character */
